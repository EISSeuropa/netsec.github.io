...
# COST evaluates an Action on its inclusiveness, and ITC participation is the
# number that carries most weight there. The Annotated Rules (Level C v3.0,
# 25 September 2025) do not hold the list: they say twice that it lives in the
# Country and Organisations Table, so these 25 are quoted from
#
#   Annex I - Level A: Country and Organisations Table, version 1.7,
#   1 November 2023
#   https://www.cost.eu/uploads/2023/11/Annex-I-level-A-Country-and-Organisations-Table-Version-1.7.pdf
#
# checked against that document on 27 August 2026, and cross-checked against
# the Excellence and Inclusiveness page on cost.eu, which lists the same 25.
# Re-check when COST publishes a new version of the annex: this is a policy
# fact aimed at an evaluator, and a list one country wrong is a wrong public
# claim about a member's institution.
#
# The same table also designates the EU Outermost Regions as ITC (French
# Guiana, Guadeloupe, Martinique, Mayotte, Reunion Island, Saint-Martin, the
# Azores, Madeira and the Canary Islands). bios.json carries a country and
# nothing finer, so the count here is by country and understates a member in
# one of them. The page says so where it shows the figure.
ITC_COUNTRIES = frozenset({
    # EU Member States
    "bulgaria", "croatia", "cyprus", "czech republic", "estonia", "greece",
    "hungary", "latvia", "lithuania", "malta", "poland", "portugal",
    "romania", "slovakia", "slovenia",
    # Full Members that are not EU Member States
    "albania", "armenia", "bosnia and herzegovina", "georgia", "moldova",
    "montenegro", "north macedonia", "serbia", "turkiye", "ukraine",
})

# bios.json does not spell every country the way the COST table does, and a
# near miss here is a silent undercount rather than an error. Czechia and
# Bosnia & Herzegovina are the two the current roster actually needs.
ITC_ALIASES = {
...
def normalise_country(name: str) -> str:
    """Lower-case, diacritic-stripped, ampersand spelled out, then aliased.

    Diacritics go so "Türkiye" matches whichever side writes it with the
    umlaut, and "&" becomes "and" because the directory writes
    "Bosnia & Herzegovina" where COST writes it out.
    """
    s = unicodedata.normalize("NFKD", str(name or ""))
    s = "".join(c for c in s if not unicodedata.combining(c))
    s = s.lower().replace("&", "and").strip()
    s = re.sub(r"\s+", " ", s)
    return ITC_ALIASES.get(s, s)


def is_itc(country: str | None) -> bool:
    return bool(country) and normalise_country(country) in ITC_COUNTRIES
...
def build(wg: dict, bios: dict | None = None, programme: dict | None = None,
    ...
    # Stamped after the person universe is settled, so both places a person can
    # be constructed are covered by one rule. Only where true, which keeps the
    # field out of every other node (#1646).
    itc_people = 0
    for person in people.values():
        if is_itc(person.get("country")):
            person["itc"] = True
            itc_people += 1
