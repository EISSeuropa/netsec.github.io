...
# The Network Map's list region (#1650) is the table of everyone the map draws,
# written into network-map.html and its two translations by
# scripts/build-network-map.py. Its rows are member names, WG numbers and
# English country exonyms, identical in all three locales and regenerated by
# every bios and cost.eu sync. One member joining the Action moved the English
# markup and falsely flagged the FR and DE pages as stale on the sync's own
# auto-PR (#1686), which is the same shape as the facepile above and the
# cache-bust queries above that.
#
# The region's own chrome (its summary, its hint, its column headers) is
# translatable, and normalising it out does not lose a guard: those strings
# live in LIST_LOCALES in build-network-map.py, which writes all three locales
# from one dict, and its --check mode already fails CI if a page does not match
# what the script emits.
_MAP_LIST_RE = re.compile(
    r'<!-- network-map:list start -->.*?<!-- network-map:list end -->', re.S
)
