...
| `data/essc-livestream.json` | The conference livestream. Set `url` to the Zoom webinar link and `start`/`end` to the conference window (ISO with a `+02:00` CEST offset). The programme page shows the "Watch live" button and per-session links only inside that window, and marks the running session "Live now". Blank `url` (or delete the file) to take it down after the conference. The daily Indico sync does not touch this file. |
