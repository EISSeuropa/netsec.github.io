...
This guide walks through creating the Google Form, linking it to a Sheet, and wiring the published Sheet into the auto-sync workflow. **One-time setup, ~30 minutes.** After this, ongoing involvement is "review the PR, click Merge".

## How the pipeline works

```
Google Form ──► Google Sheet ──► sync-bios.yml (daily) ──► PR ──► merge ──► /people/
```
...
|---|---|---|---|
| 1 | **Title** | Dropdown — options exactly: `Prof.` / `Dr` / `Ms` / `Mr` / `Mx` / `None please` | ✅ |
...
| 9 | **Mentorship (optional)** | Checkboxes: *Open to mentoring early-career researchers · Looking for a mentor · I am currently mentoring at full capacity · I found a mentor through this directory* | ⬜ |
...
> **Title + name split.** The honorific is its own **Title** dropdown (`Prof.` / `Dr` / `Ms` / `Mr` / `Mx` / `None please`), kept separate from **Full name** so submitters never type the title into the name field. Typing it inline used to leak the honorific into the member's slug and profile-page URL when the title was spelled out in full ("Professor Mark Rhinard" → `professor-mark-rhinard`). `scripts/sync-bios.py` (`build_name`) prepends the chosen title to the name, drops `None please`/blank to no honorific, and folds any honorific a submitter still typed into the name field so it is never duplicated. The column headers live in `scripts/bios-source.json` (`title` → "Title", `name` → "Full name"); the old single-field header is kept as `name_legacy` so responses captured before the split still resolve and a renamed question never blanks an existing member.
>
> **Name validation + sync-side guards.** The *Full name* question carries a Google Forms response-validation rule: **minimum 3 characters**, with a custom error message, so a stray token is rejected at submission. That is the first line of defence. Backstops run in `scripts/sync-bios.py` for anything the form lets through: a row whose name is **only a title** ("Mr", "Dr.") is dropped (#908), and a title **glued to the name by a dot with no space** ("Mrs.Yanina") is stripped in `slugify` and `name_key` so the submission collapses onto its twin rather than forming a second card. The sync also **standardises the leading honorific to house style** (`normalise_title`): Mr / Mrs / Ms / Mx / Dr carry no full stop, Prof carries one ("Prof."), the written-out forms fold to the abbreviation ("Professor" → "Prof.", "Doctor" → "Dr"), regardless of how the member typed it. A real name that merely starts with a title's letters ("Drew") is left alone. If you change the minimum or the error text on the form, note it in the table above so the form can be recreated faithfully.
...
> **Mentorship parsing.** The script reads the four checkbox options into a `mentorship` list of role tags. *"Open to mentoring …"* parses to `mentor`, *"Looking for a mentor"* to `mentee`, *"I am currently mentoring at full capacity"* to `mentor-full`, and *"I found a mentor through this directory"* to `matched`. A member can tick any combination, or none. The match is a tolerant substring check on the cell text, so light rewording is safe as long as an offering option still says *mentoring* / *as a mentor*, a seeking option still says *looking for* / *seeking a mentor*, a capacity option still says *at capacity* / *full capacity*, and a match option still says *found a mentor*. The directory badge labels are recognised too (*"Available to mentor"* → `mentor`, *"Seeking mentorship"* → `mentee`, *"Mentoring, at capacity"* → `mentor-full`), so if you edit the Sheet by hand you can type the status as it appears on the card rather than the exact Form option. Put the senior / mid-level framing (mentors are typically senior or mid-level scholars) in the question's **description** text, not in the option labels: it stays guidance for respondents and never reaches the parser. The field is dormant until the question is live and members resubmit. An absent column parses to an empty list and renders nothing.

> **The last two options are the off-switches (#1416).** `merge()` only overwrites a field when the incoming value is non-empty, so a member who unticks a box and resubmits keeps the old flag standing. *"I am currently mentoring at full capacity"* and *"I found a mentor through this directory"* give them something positive to tick instead. The re-parsed cell replaces the whole stored list, and `mentor-full` suppresses `mentor` while `matched` suppresses `mentee`, which covers the member who ticks the new box but leaves the old one ticked as well. Neither new tag is matched by any filter chip or by the matching wizard, so carrying one takes the member out of the pool on its own. `mentor-full` renders as a muted, non-clickable *"Mentoring, at capacity"* pill that keeps the member visible as a mentor without inviting an intro mail. `matched` renders nothing at all, since the member is simply no longer seeking. For an impact count of matches made through the directory, read the tag straight out of `data/bios.json`:
>
> ```bash
> jq '[.members[] | select(.mentorship // [] | index("matched"))] | length' data/bios.json
> ```
