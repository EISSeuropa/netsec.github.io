...
| P1 | 2 / Lighthouse | `index.html` | P1 | Performance score 71 on home (target 80). Largest offenders: render-blocking requests, image delivery (~900 KB savings possible). | Local-loopback Lighthouse skews lower than CDN-served (the live site doesn't hit the cache + compression that GitHub Pages applies). Re-test on `https://netsec-cost.eu` over real network — likely 85+. If still under target, defer image optimisation to v1.5.0. | deferred (validate against live site) |
| P2 | 2 / Lighthouse | `press-kit.html` | P1 | Performance score 62. LCP 8.3 s local; large logo + poster images dominate. | Same caveat as P1. Press-kit is image-heavy by design; if live Lighthouse still red, queue image optimisation pass. | deferred (validate against live site) |
| P3 | 2 / Lighthouse | `people.html` | P1 | Performance score 72. Directory cards + Pagefind scripts on first load. | Same caveat. | deferred (validate against live site) |
| P4 | 2 / Lighthouse | `grants.html` | P1 | Performance score 74. Likely similar to home. | Same caveat. | deferred (validate against live site) |
