...
<!-- AUTOSTAMP:BEGIN -->
> _Auto-tracked: `[Unreleased]` is empty since **v1.8.0**. Last refresh by `scripts/sync-roadmap.py`: 25 May 2026._
<!-- AUTOSTAMP:END -->
...
| Late Jul – mid Aug 2026 | 📅 **v1.8.1** (patch) | FR/DE native-speaker translation review of FAQ + Glossary (lifts beta) · post-conference + Summer School recap on news · search acronym synonyms (STSM ↔ Short-Term Scientific Mission, etc.) · D8 (STSM planning guidelines) hosted on Grants page · cross-browser audit + pre-plenary accessibility pass · FR/DE em-dash sweep ([#164](https://github.com/EISSeuropa/netsec.github.io/issues/164)) · `promote-roadmap.py` structural fix so the next minor's shipped card derives from CHANGELOG ([#233](https://github.com/EISSeuropa/netsec.github.io/issues/233)). No new feature; all content + quality + release-infrastructure work. |
...
| Mid Oct 2026 | 📅 **v1.9.0** | Year 1 retrospective on news · Outputs section refresh with D6 cards + `schema.org/ScholarlyArticle` · Phase 2 IA homepage restructure · per-event `.ics` files + Add-to-calendar buttons on event cards · news RSS feed at `/news.xml` · D10 (risk management strategy) hosted. (Absorbs the deferred-from-v1.8.0 calendar / RSS / D10 items.) Pre-plenary cut. |
| Late Dec 2026 | 📅 **v1.10.0** | D11 + D12 hosting with stable permalinks and citation metadata · per-page Open Graph images · print stylesheet for FAQ + Glossary · Year 2 readiness. Final release of 2026. |
