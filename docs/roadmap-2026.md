...
Last revised **4 June 2026**, reflecting three plan changes. v1.10.0 shipped 31 May, ahead of its 5 June target. v1.10.1 pulled forward to 9 June and reframed from a post-conference recap into the quality and content patch it actually became, with the ESSC recap itself moving to v1.11.0. v1.11.0 holds at early July. Six planned releases now interleave with the Action calendar: **v1.8.1** shipped **27 May** as a quality patch covering the work accumulated since v1.8.0 (founding contributors section, home-page events refresh, release-infrastructure hygiene, `sync-cost.py` per-bio WG sync, beta-translation ribbon language-switch fix, FR / DE prose em-dash sweep). **v1.9.0** shipped **29 May** (cut roughly a week early), ahead of the Summer School + ESSC in Stockholm: per-event `.ics` files at `/calendar/<slug>.ics` driving a per-card *Add to calendar* dropdown (Google / Outlook / Apple webcal / direct .ics), news RSS feed at `/news.xml` (sourced from a new `data/news.json`), and a structural refactor so home-page event and news cards derive from JSON instead of hand-coded HTML across three locales — including a five-line clamp + Read more affordance for longer descriptions ([#249](https://github.com/EISSeuropa/netsec.github.io/issues/249)). D10 if ready. **v1.10.0** (shipped **31 May**, ahead of the 5 June target) version-tagged what had already merged since v1.9.0: ESSC co-author bylines, the roadmap-card automation, the Indico-sync change summary, and the sync-cost Gap C fix. The co-author feature makes it a minor, not a patch. **v1.10.1** (**9 June**) gathers the quality and content work accumulated since v1.10.0: the accessibility statement refresh to v1.3, the LinkedIn and Bluesky footer links, the directory Early Access banner, the About-page deliverable statuses, the Working Groups page sections, the FR/DE FAQ bio-update clarification, and the remaining sync-cost Gap B. The Summer School and ESSC recap moves to v1.11.0 once the conference wraps. **v1.11.0** then pulls forward to **early July** (it was the mid-September pre-plenary cut; the inaugural MC plenary on the 18th now ships no dedicated release): directory mentorship (profile-card badge + filter, [#498](https://github.com/EISSeuropa/netsec.github.io/issues/498)), keyword proper-noun + affiliation data-quality fixes ([#505](https://github.com/EISSeuropa/netsec.github.io/issues/505), [#506](https://github.com/EISSeuropa/netsec.github.io/issues/506)), Outputs section refresh with D6 cards + `schema.org/ScholarlyArticle`, Phase 2 IA pass, founding-cohort follow-ups ([#245](https://github.com/EISSeuropa/netsec.github.io/issues/245)), PDF documentation pack section-level catch-up ([#229](https://github.com/EISSeuropa/netsec.github.io/issues/229)), D8 if approved. The Summer School and ESSC recap, a news-block gallery and a short feedback summary, also lands here once Stockholm wraps. The stretch from August to November is a deliberate break. **v1.12.0** closes the year in **late December**: Year 1 retrospective, D11 + D12 hosting, per-page OG images, FAQ / Glossary print stylesheet, member-photos-out-of-git refactor ([#119](https://github.com/EISSeuropa/netsec.github.io/issues/119)). The per-release narrative further down still lags from the v1.4.0–v1.8.0 cycle; that catch-up is queued ([#229](https://github.com/EISSeuropa/netsec.github.io/issues/229)) and the public CHANGELOG remains authoritative for shipped scope and dates.

<!-- AUTOSTAMP:BEGIN -->
> _Auto-tracked: **56 entries** in [`[Unreleased]`](../CHANGELOG.md#unreleased) since **v1.10.0** (12 Added, 20 Changed, 24 Fixed). Last refresh by `scripts/sync-roadmap.py`: 8 Jun 2026. Prose in the timeline below may lag; the maintainer resynthesises on release-time §5 sweep._
<!-- AUTOSTAMP:END -->
...
| 9 Jun 2026 | 📅 **v1.10.1** (patch) | Quality and content patch shipping as the Stockholm week opens: accessibility statement refreshed to v1.3 (June re-assessment) · LinkedIn + Bluesky footer links ([#63](https://github.com/EISSeuropa/netsec.github.io/issues/63)) · new hand-designed social cards (a general card plus a dedicated directory card) · directory Early Access Preview banner · About-page deliverables timeline showing shipped versus planned · per-Working-Group *Working towards* deliverables + *Related publications* sections · FR/DE FAQ bio-update clarification ([#183](https://github.com/EISSeuropa/netsec.github.io/issues/183)) · the remaining sync-cost Gap B statistics + country-roster refresh ([#236](https://github.com/EISSeuropa/netsec.github.io/issues/236)). The Summer School + ESSC recap (news-block gallery + feedback summary, recordings link where available) moves to **v1.11.0** once the conference wraps. |
| 9–12 Jun 2026 | 📅 *Stockholm* | Early-Career Scholars Summer School (9–11 Jun) + European Security Conference (11–12 Jun) |
| Early Jul 2026 | 📅 **v1.11.0** | Early-summer feature cut, led by directory mentorship: members flag in the join form that they are available to mentor early-career researchers or seeking mentorship, surfaced as a profile-card badge + a directory filter, all three locales ([#498](https://github.com/EISSeuropa/netsec.github.io/issues/498)) · directory data quality: research-keyword proper-noun capitalisation ([#505](https://github.com/EISSeuropa/netsec.github.io/issues/505)) + affiliation punctuation uniformity ([#506](https://github.com/EISSeuropa/netsec.github.io/issues/506)) · Outputs section refresh with a real card design for D6 policy briefs + `schema.org/ScholarlyArticle` metadata · Phase 2 information-architecture pass (homepage + header, audience tracks, mobile patterns, content lifecycle) · founding-cohort follow-ups: per-bio *founding contributor* badge + founding stats on press kit ([#245](https://github.com/EISSeuropa/netsec.github.io/issues/245) Items C + D) · PDF documentation pack section-level catch-up: site graph + screenshots refreshed ([#229](https://github.com/EISSeuropa/netsec.github.io/issues/229)) · D8 (STSM planning guidelines) hosted on Grants page if approved · ESSC programme download button + print-to-PDF of the live grid ([#318](https://github.com/EISSeuropa/netsec.github.io/issues/318)) · fix the Indico write tooling so fix-plans actually apply against the live instance ([#323](https://github.com/EISSeuropa/netsec.github.io/issues/323)) · Summer School + ESSC recap on the news block (gallery + feedback summary, recordings link where available), carried over from v1.10.1 once Stockholm wraps. |
...
| Late Dec 2026 | 📅 **v1.12.0** | Year 1 retrospective on news · D11 (inclusion & diversity declaration) + D12 (environmental sustainability guidelines) hosted with stable permalinks and citation metadata · per-page Open Graph images · print stylesheet for FAQ + Glossary · member-photos-out-of-git refactor so repo size stays linear-in-text rather than linear-in-headshots ([#119](https://github.com/EISSeuropa/netsec.github.io/issues/119)) · Year 2 readiness. Final release of 2026. |
