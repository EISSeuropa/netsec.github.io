...
### Indico patch helper (Phase 1)

- **`scripts/indico_patch.py`** — write-side companion to the daily `sync-indico.py`. Reads a tiny YAML "fix-plan" describing metadata corrections (session rename, room reassignment, contribution session-move, person affiliation change, session-block timing), resolves friendly Indico IDs to internal database IDs against the live read API, then dispatches the right write call: JSON `PATCH` for clean cases (contribution session-move, block time, person affiliation), wtforms `POST` for legacy paths (session-modify, contribution-edit). Dry-run by default; `--apply` flips to live writes. Resolved internal IDs are cached in a gitignored sidecar JSON next to each YAML so re-runs skip the lookup step and the YAML stays comment-friendly. Token validation on startup (`GET /api/user/`) fails fast if the `INDICO_WRITE_TOKEN` env var is missing the `full:everything` scope. Schema documentation lives at `data/indico-fix-plans/EXAMPLE.yaml`; workflow notes at `data/indico-fix-plans/README.md`; design rationale at `docs/indico-patch.md`. Phase 1 validates the two JSON-PATCH endpoint families end-to-end against the live API (resolution + dispatch shape both confirmed); Phase 1.5 will validate the three wtforms paths once a write-token is in hand for live exercise. Built to make ESSC 2027's prep cycle 10× faster after the ESSC 2026 audit (#208) showed how much manual UI clicking the drift between source-of-truth PDF and Indico requires. Tracks #210.
