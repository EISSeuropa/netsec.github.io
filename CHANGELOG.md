...
- The mobile directory filter sheet now pins correctly to the viewport bottom on both iOS Safari and Chrome iOS. The previous scroll-lock used `body{position:fixed}`, which works on Safari but on Chrome iOS promotes the body as a containing block for `position:fixed` children, placing the sheet mid-page instead of at the viewport bottom. The same stacking-context split caused the backdrop to partially cover the sheet header, making the close button require multiple taps. The scroll lock now intercepts `touchmove` events instead, calling `preventDefault()` on any touch outside the sheet without touching layout. All three locales.
