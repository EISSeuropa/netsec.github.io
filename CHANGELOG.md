...
### Visual identity · NetSec logo deployment

- **Designer-provided NetSec logo deployed across all 46 HTML pages.** Replaces the launch-era "NS" gradient-square placeholder with the new four-petal mark + wordmark. Three deliverables in one PR:
    - **Header brand link.** Two `<img>` elements (coloured lockup + white-on-dark lockup) keyed off the site's `.dark` class via CSS visibility, so the logo follows whatever theme the visitor explicitly picked rather than the OS `prefers-color-scheme` (which would desync against the rest of the page). On viewports below 700px, both lockups hide and a 32×32 mark-only image replaces them — saves header real estate against the hamburger + language switcher + theme toggle. We deliberately ship three distinct PNGs rather than CSS `filter: invert()` because inversion destroys the petal gradient. The `<a aria-label="NetSec home">` carries the brand name for screen readers; the `<img>`s themselves have `alt=""` to avoid double-announcement.
    - **Favicon family.** Replaces the placeholder `favicon.svg` with the per-size PNG chain (16, 32, 48 for browser tabs; 180 for Apple touch-icon; 192 and 512 for the Android home-screen and PWA manifest), plus a multi-resolution `favicon.ico` for legacy clients. All rasterised from the designer's 595×599 `imageonly.png` mark. New `manifest.webmanifest` at repo root carries the Android/PWA icon references plus `theme_color` and `background_color` so the OS shortcut UI matches the brand.
    - **OG / Twitter social card.** New `assets/images/og-image.png` at 2400×1260 (matching the previous placeholder dimensions, no HTML change needed): primary lockup centred on a soft brand-tinted white canvas. Affects link previews on LinkedIn, Mastodon, Bluesky, Slack, Twitter, and Facebook.
    - JSON-LD `Organization.logo` updated to point at the new 512×512 mark — improves Google Knowledge Panel rendering.
- **Reproducible build pipeline.** Two one-off scripts (kept in repo for future logo refreshes): `scripts/build-brand-assets.py` rasterises + crops the designer's source PNGs into the deployment set, and `scripts/update-brand-html.py` applies the three targeted replacements (favicon chain, brand element, JSON-LD URL) across all HTML pages.
- **What's not in scope.** Migrating the site accent colour `#003399` → brand `#2B639C` is deferred — the values are close but not identical, and the migration is cross-cutting (site.css + JSON-LD theme-color + manifest theme_color + several inline `<style>` blocks). SVG masters of the lockup were not delivered with the package; favicons rasterised from the 595px mark look acceptable but suboptimal at 16px. Both flagged on #220 for follow-up. Closes #220.

### Indico patch helper (Phase 1 + 1.5)

- **`scripts/indico_patch.py`** — write-side companion to the daily `sync-indico.py`: reads a YAML "fix-plan" describing metadata corrections (session rename, room change, contribution session-move, person affiliation, block-time edit), resolves friendly Indico IDs to internal database IDs against the live read API, then dispatches the right write call. Dry-run by default; `--apply` flips to live writes. Resolved IDs cached in a gitignored sidecar JSON. Schema docs at `data/indico-fix-plans/EXAMPLE.yaml`; workflow notes at `data/indico-fix-plans/README.md`; design rationale at `docs/indico-patch.md`.

- **Phase 1.5 unlock — admin flag on the bot account.** Four probe rounds against the live EISS Indico instance (PRs #212-#216) walked the permission ladder to find what unlocks Bearer-token writes on the `/event/<id>/manage/*` routes. Read-only scope on the bot's PAT → 403 (anonymous-session pattern). Scope upgraded to `full:everything` + every other box ticked → identical 403; scope alone isn't the gate. Admin flag flipped on the bot's user account on Indico → **200 OK** across all probed management endpoints (session-modify returns `{html, js}` JSON with current form values; contribution REST OPTIONS confirms `PATCH, DELETE`; wtforms `/edit` endpoints accept `POST`). The 403-with-anonymous-session pattern we initially read as "Bearer auth ignored" turned out to be the standard auth-then-permission flow: Indico processed the token, found the user lacked management permission, and fell through to the anonymous render path. The probe script + workflow are removed (one-shot diagnostics, served their purpose; results captured in `docs/indico-patch.md` and on #210). Operational precondition is now documented: the bot account owning `INDICO_WRITE_TOKEN` needs the admin flag set on Indico. Live no-op write validation deferred to first real ESSC 2027 prep apply (one open ID-namespace subtlety on contribution REST PATCH to resolve). Tracks #210.

- **`scripts/indico_clean_duplicate.py`** — companion to `indico_patch.py` for the ESSC-N → ESSC-N+1 rollover. Indico's "duplicate event" feature carries the previous year's contributions, sessions, and materials along with the configuration (review workflow, custom fields, registration form, etc.) we actually want to inherit. New submissions then continue the old friendly-ID counter — your first ESSC 2027 abstract becomes #342 instead of #1. This script lists inherited content via the read API and selectively `DELETE`s it via the management API, leaving configuration intact. Safety net: a hardcoded `PROTECTED_EVENTS` allow-list refuses to touch the live ESSC 2026 (event 22) unless `--force` is passed. Dry-run by default; explicit `--delete <category>` required per content type (no "delete everything" shortcut). Smoke-tested in dry-run against event 22 — enumerated 105 contributions correctly via the read API and produced the right DELETE URLs without issuing any. Use case + workflow documented in `docs/indico-patch.md` under "Companion tool". Tracks #210; same admin precondition as `indico_patch.py`.
