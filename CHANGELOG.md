...
- **FAQ copy edit: voice-rule sweep, a removed entry, and a headshot-update warning.** Across `/faq.html` (+ FR + DE): the prose semicolons were rewritten as full stops or commas per the voice rules; the *Why is the site so minimalist?* entry was removed; and *How do I update my bio, photo, or affiliation later?* now warns that a new headshot goes through a separate form (not the main bio form) and reassures that resubmitting keeps existing data and only needs the required fields.
