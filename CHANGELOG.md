...
- **Beta-translation ribbon overlapped the floating header on mobile.** On narrow viewports the long French / German sentence + link wraps to two or three lines, making the ribbon ~60–100 px tall — but the CSS used a fixed `body { padding-top: 38px }` and `.nav { top: 52px }` that only fit a single-line desktop ribbon. The nav bubble drew on top of the bottom of the wrapped ribbon. Fix: the offsets are now `var(--ribbon-h, 38px)` and `calc(var(--ribbon-h, 38px) + 14px)`, with a tiny JS handler in `site.js` that measures the ribbon's real `offsetHeight` and writes it to `--ribbon-h` on `<html>`. The handler re-runs on `window.load` (covers the case where defer-script execution beats CSS application and the first measurement returns 0), on every viewport `resize`, and via a `ResizeObserver` on the ribbon itself — so the layout offset stays in step whether the ribbon wraps to one line or four.

- **Directory bio cards never showed the full biography text** because the *Show more / Show less* toggle was never inserted on overflowing bios. The detection logic was wrapped in `requestAnimationFrame` on the assumption that it had to wait for layout before reading `scrollHeight`. That assumption was both unnecessary (reading `scrollHeight` already forces a sync layout in every modern browser) and unreliable: the rAF callback was observed firing too early on the user's browser — at a moment when `scrollHeight === clientHeight` because the four-line clamp hadn't visibly settled yet — and not firing at all in some preview environments where the tab is treated as background. Symptom in production: long bios stayed at four lines with no expand affordance, so visitors could never read the rest. Fix: drop the rAF wrapper and run the overflow check synchronously after the cards are appended. Belt-and-braces: each new bio is also observed by a single shared `ResizeObserver` so any later metric change (web-font swap, theme toggle, viewport resize) re-runs the check. The bug surfaced after PRs #98 + #100 added two more long bios to `bios.json` (John Helferich + Moritz Weiss), bringing the count of bios long enough to overflow from 2 to 4 — making the missing toggle visible across the directory rather than on the occasional card. EN / FR / DE.

### Changed

- **Pagefind search index now built at deploy time, not committed to main.** The recurring "PR conflicts on `pagefind/pagefind-entry.json`" pain (hit on PRs #98 and #100) had a single structural cause: two parallel PRs that both touched HTML each rebuilt the index, producing different content-hashed shard filenames and a conflicting manifest. Fix: move the build to a new GitHub Pages workflow (`.github/workflows/pages-deploy.yml`) that runs on push-to-main, rebuilds `/pagefind/` fresh, and deploys the working tree (plus the freshly-built index) via `actions/upload-pages-artifact` + `actions/deploy-pages`. `/pagefind/` is now gitignored — PRs never touch it, so the conflict source is gone. `search-drift.yml` simplified to a build-sanity check (per-locale page counts > 0) that catches the realistic regressions (missing `data-pagefind-body` markers, builder regressions) without needing a committed reference. `scripts/build-search.sh --check` deprecated but still runs (no-op shim that just builds). **One-time maintainer action required after this lands**: Settings → Pages → Source → switch from *Deploy from a branch* to *GitHub Actions*. Documented in the workflow file's header and in the PR description.

### Added

- **Roadmap signposted on the home page and the About page** (EN / FR / DE). The footer link alone wasn't doing the discoverability work — most visitors don't scan footers. Two prominent placements added: (1) **4th card in the *Find out more* grid** at the end of the home-page About section, sitting alongside FAQ / Glossary / Press kit with a route-map icon and the one-line lede *"What's shipped, what's underway, and what comes next for the website and the open directory."* — high-traffic surface for first-time visitors; (2) **Accent callout at the foot of the *Deliverables* section on `/about.html`**, framing the public roadmap as the website-and-directory counterpart to the Action's MoU deliverables Gantt above. The callout has a clear EU-blue *See the roadmap* CTA so stakeholders reading the Action overview can hop straight in. The home-page grid layout (`grid-template-columns: repeat(auto-fit, minmax(220px, 1fr))`) flows the new card cleanly — 4-up on desktop, 2-up on tablet, 1-up on mobile.
- **Expand / collapse chevron on directory cards in compact view** (`/people.html`). Visitors had no visual signal that compact cards could be clicked open. Small circular chevron at the bottom-right of every card — ▼ when the card is collapsed, rotates to ▲ when expanded, hidden entirely in detailed view (where there's no toggle behaviour). `pointer-events: none` so the chevron never absorbs the click; the whole card stays the hit target. Touch-friendly (always visible, not hover-gated), respects `prefers-reduced-motion` for the rotation transition. EN / FR / DE.
