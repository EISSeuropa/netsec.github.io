...
- **News RSS feed at `/news.xml` + structured news data layer (v1.9.0 Phase 3).** New `data/news.json` is the source of truth for both the home-page news cards and the public RSS feed. Schema mirrors `data/events.json`'s shape per item: `id`, `pubDate` (ISO 8601 with offset), `displayDate.{en,fr,de}` (the pill label), `title.{en,fr,de}`, `body.{en,fr,de}`, optional `cta.{href, external, i18n.{en,fr,de}}`. `scripts/build-news-rss.py` generates `news.xml` in RSS 2.0 format (single-language EN per RSS convention; FR / DE feeds are a deferred follow-up if reader demand surfaces) with `<atom:link rel="self">`, per-item CDATA-wrapped descriptions, `guid isPermaLink="false"`, and RFC 822 `pubDate`. The matching `news-drift.yml` CI workflow runs `--check` on every PR touching `data/news.json` / `news.xml` / the generator, mirroring the `calendar-drift` workflow shape. A new `assets/js/home-news.js` renderer reads the JSON on `DOMContentLoaded`, picks the locale, sorts newest-first, and rebuilds `#news .news-list`; the hand-coded HTML on `index.html` (+ FR + DE) survives as a fail-soft fallback. Every home page now carries `<link rel="alternate" type="application/rss+xml" href="https://netsec-cost.eu/news.xml">` in `<head>` for feed-reader auto-discovery (locale-specific feed titles per page). Closes the news-from-JSON half of [#249](https://github.com/EISSeuropa/netsec.github.io/issues/249)'s sibling refactor scope.
...
- **Home-page event cards now derive from `data/events.json`** (closes the renderer half of [#249](https://github.com/EISSeuropa/netsec.github.io/issues/249)). New `assets/js/home-events.js` reads the events JSON on `DOMContentLoaded`, picks the locale from `<html lang>`, and rebuilds the `#events .event-list` cards from structured data. Schema extends each event with `eventType`, `featured`, `displayDate`, `cardTitle`, `cardDescription`, `meta[]`, and `cta` — all with `{en, fr, de}` blocks where appropriate. The pre-existing hand-coded HTML survives as a fail-soft fallback (kept verbatim across `index.html` + FR + DE) if the fetch or parse fails; the renderer replaces it on success. Card descriptions get a **five-line clamp with a *Read more* / *Lire la suite* / *Mehr anzeigen* toggle** that's only injected when the rendered text actually overflows; `@media print` drops the clamp so the print layout carries full text. Each card carries an **Add to calendar dropdown** with four destinations: Google Calendar (prefilled template), Outlook web compose deep-link, Apple `webcal://` subscription, and direct `.ics` download from the per-event files added in v1.9.0 Phase 1. Outside-click + Escape dismiss the menu. The menu reparents itself to `<body>` on first open and pins with `position: fixed` against the trigger's bounding rect, so it escapes the stacking context that `.event-card` creates via `backdrop-filter` (otherwise the menu was occluded by the next card down — same portal pattern as the ESSC member-preview popover). Page scroll / window resize closes the menu rather than tracking the trigger, matching the existing popover dismissal convention. Sidesteps the FR / DE drift class that #248 had to paper over by hand. The Indico-write tooling continues to overwrite `summary` / `start` / `end` per `indicoEventId`; the renderer prefers `cardTitle.{locale}` over `summary` so localised home-page titles aren't affected.
