...
- Closing the mobile filter sheet is now instant on iOS Safari. The dialog's `::backdrop` carried a `backdrop-filter: blur`, and blurring a full viewport of the directory grid is GPU-expensive on iOS Safari, so tearing it down on close cost about half a second of recomposite that read as a laggy dismiss (Chrome iOS composited it without the hitch). The blur is dropped; the 50% dim alone carries the modal weight, and a 2px blur under it was imperceptible anyway.
