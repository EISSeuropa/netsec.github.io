...
- The directory's *Add your bio* card and guided-tour step now say a new entry appears "within a week of submission" rather than the ambiguous bare "within a week", in all three locales, so the timing is explicit. The FAQ's "How do I join the directory?" answer now says the form takes about three minutes, matching the card.
...
- The directory's filters now lead with people rather than chrome, in all three locales. The three secondary facets (research themes, research regions, mentorship) collapse into a "More filters" disclosure that stays closed by default and lives inside the filter card, with a badge counting how many secondary filters are active; a deep-link (`#themes=…`) or a click on a card's keyword pill opens it automatically. The result count ("N / M members") and a global "Clear all filters" button sit just above the grid, after all the filters, so the number reflects what is on screen. The density toggle, guided tour and add-your-bio buttons move out of the filter card into a pinned bar at the corner of the directory, so they stay reachable while scrolling. The research-regions heading gains a one-line note that it means where a member focuses their research, not where they are based; the search box reads with more contrast in light mode; and its placeholder is shortened so it no longer visually truncates. The guided tour now scrolls each step into view and opens the "More filters" panel when a step lands inside it, so it works correctly even when launched from the pinned tools bar after scrolling down.
