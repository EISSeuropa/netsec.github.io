...
- **Working Groups data layer (`data/wg.json`), synced from cost.eu.** Groundwork for a dedicated Working Groups page (issue [#378](https://github.com/EISSeuropa/netsec.github.io/issues/378)). `scripts/sync-cost.py` now writes a fourth surface: a per-WG dataset giving each of the four groups its lead and co-lead (read from each bio's `wg_leadership`), its directory members (from cost.eu's Membership table), and a count of members without a bio yet. WG titles and the colour palette are config in the script; the file regenerates every weekly sync and is never hand-edited, so a leadership or membership change on cost.eu flows straight through. Covered by a new case in `scripts/test-sync-cost.py`. No visible change yet, the page that renders it follows.
