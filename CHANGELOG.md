...
- The mobile directory filter sheet is now a proper modal for assistive technology. While it is open the rest of the page is set `inert`, so keyboard focus and VoiceOver stay inside the sheet instead of slipping behind the dimmed backdrop, and the sheet carries `role="dialog"` with `aria-modal` and a name drawn from its "Filters" heading. The roles apply only while the sheet is open, since the same controls render inline on desktop. All three locales.
...
- The mobile directory filter sheet now pins correctly to the viewport bottom on both iOS Safari and Chrome iOS, and filter button taps are immediate. The previous scroll-lock chain was: `body{overflow:hidden}` breaks Safari (body becomes a scroll container, knocking fixed descendants off-viewport); `body{position:fixed}` fixes Safari but breaks Chrome iOS (body promoted as containing block, sheet lands mid-page); a `touchmove` listener with `passive:false` fixes Chrome iOS but forces Safari to run JavaScript before resolving every touch interaction, adding a noticeable delay to taps. The scroll lock is now CSS-only: the backdrop carries `touch-action:none` (blocks scroll-through in the dimmed area) and the sheet carries `touch-action:pan-y` with its existing `overscroll-behavior:contain` (confines sheet scroll to the sheet). No body style changes, no passive listeners. All three locales.
- The mobile directory filter sheet now stays pinned to the bottom of the screen on iOS Safari. Locking the page scroll with `body{overflow:hidden}` turned the body into a scroll container, which on iOS knocks `position:fixed` descendants (the sheet and its dimming backdrop) off the viewport, so after scrolling down and opening Filters the sheet appeared mid-page over the member cards with no dimming. The scroll lock now freezes the body with `position:fixed` and a restored offset instead, and the page-level horizontal-overflow guard uses `overflow-x:clip` rather than `hidden` to avoid the same scroll-container side effect. All three locales.
