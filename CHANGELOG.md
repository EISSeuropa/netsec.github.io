...
- **ESSC *Speakers & chairs* guidance moved up and made compact.** The speaker self-service block on `/essc-2026.html` (+ FR + DE) moved from the foot of the practical-information section to a slim callout directly under the conference header, where speakers and chairs land first. It is a single line (icon plus a one-sentence pointer to the conference FAQ) rather than a full practical block, so it catches the target audience without distracting other visitors.
