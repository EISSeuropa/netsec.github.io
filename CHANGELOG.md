...
### Indico patch helper (Phase 1, writes blocked)

- **`scripts/indico_patch.py`** — designed as the write-side companion to the daily `sync-indico.py`: reads a YAML "fix-plan" describing metadata corrections (session rename, room change, contribution session-move, person affiliation, block-time edit), resolves friendly Indico IDs to internal database IDs against the live read API, then dispatches the right write call. Dry-run by default; `--apply` flips to live writes. Resolved IDs cached in a gitignored sidecar JSON. Schema docs at `data/indico-fix-plans/EXAMPLE.yaml`; workflow notes at `data/indico-fix-plans/README.md`; design rationale + Phase 1.5 findings at `docs/indico-patch.md`.

- **Phase 1.5 — Indico API probe.** Two probe rounds against the live EISS Indico instance established that **Personal Access Tokens cannot reach `/event/<id>/manage/*` routes at any scope** (not even `full:everything` plus every other available box ticked). Management routes return 403 with `Vary: Cookie` + a fresh `Set-Cookie` and no `WWW-Authenticate` header — Indico ignores the Bearer header on those paths and falls back to anonymous session-cookie auth. The `/api/user/` whoami works fine, confirming the token itself is valid. This matches documented Indico behaviour: the management UI checks `g.flask_login_user` (session cookie), not `g.current_user` (any auth source). The probe script + workflow have been removed (one-shot diagnostic, served their purpose; results captured in `docs/indico-patch.md` and on #210). The patch helper is in **dry-run-only mode** until OAuth-client-app or service-account auth is configured by an Indico admin — separate work, tracked as Phase 2 of #210. The dry-run output is still useful as a deterministic, auditable checklist of UI clicks a human needs to make. Tracks #210.
