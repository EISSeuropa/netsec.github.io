...
- Removed the directory's card-density switcher. The grid now takes its density from the viewport: full photo-and-bio cards on desktop, compact cards (initials, name, affiliation and Working-Group chips) on phones, where a single column of detailed cards ran too long to scroll past. The mobile bottom-sheet work had already hidden the toggle on small screens; this drops the control on every screen and removes its guided-tour step. All three locales.
