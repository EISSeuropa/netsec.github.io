...
- The directory's filters now lead with people rather than chrome, in all three locales. The result count ("N / M members") and a global "Clear all filters" button sit in a results bar directly under the search box, and the three secondary facets (research themes, research regions, mentorship) collapse into a "More filters" disclosure that stays closed by default, so the member grid rises up the page instead of sitting below a stack of filters. A small badge on the disclosure counts how many secondary filters are active, and a deep-link (`#themes=…`) or a click on a card's keyword pill opens it automatically. The research-regions heading also gains a one-line note that it means where a member focuses their research, not where they are based, and the search placeholder is shortened so it no longer visually truncates.
